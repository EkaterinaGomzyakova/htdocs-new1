...
<script>
    var IPOLSDEK_pvz = {
        ...
        Y_selectPVZ: function(wat){
            IPOLSDEK_pvz.Y_map.setCenter([IPOLSDEK_pvz.cityPVZ[wat].cY,parseFloat(IPOLSDEK_pvz.cityPVZ[wat].cX)]);
            $('#SdekMap').on('shown.bs.tab', function (e) {
                setTimeout(function () {
                    IPOLSDEK_pvz.cityPVZ[wat].placeMark.balloon.open();
                }, 200);
                $('#SdekMap').off('shown.bs.tab');
            });
            $('#SdekMap').tab('show');
        },
        ...
        makeSearch: function () {
            $('#SDEK_search_input').on('keyup', function(e) {
                $('.SDEK_all-items .jspPane p').each(function(key) {
                    if($(this).text().toUpperCase().indexOf($('#SDEK_search_input').val().toUpperCase()) >= 0) {
...
<div id='SDEK_pvz'>
    <div class="custom-template">
        <div id='SDEK_head'>
            <div class="custom-logo-sdek"></div>
                <div class="custom-logo-name-sdek">пункты самовывоза</div>
            <div id='SDEK_closer' onclick='IPOLSDEK_pvz.close()'></div>
        </div>
            <ul class="nav nav-tabs">
                <li class="nav-item active">
                    <a class="nav-link" data-toggle="tab" href="#wrap-SDEK_info">Списком</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="SdekMap" data-toggle="tab" href="#SDEK_map">На карте</a>
                </li>
            </ul>
            <div class="tab-content">
                <div class="tab-pane fade active in" id="wrap-SDEK_info">
                        <form id="SDEK_search-wrapper" action="" method="get">
                            <div id="custom-form-control" class="form-control">
                                <input id="SDEK_search_input" type="text" placeholder="ведите адрес пункта самовывоза">
                            </div>
                            <div class="custom-SDEK_looper"></div>
                        </form>
                    <div id='SDEK_info'>
                ...
                <div class="tab-pane fade" id='SDEK_map'></div>
            </div>
    </div>
</div>
