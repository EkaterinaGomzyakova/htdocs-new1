...
BX.ready(function () {
	...
	if (ClanbeautyFrontOptions.basket_comment_is_active == 'Y') {
		$('#santa-request').val(BX.getCookie('santa-request'));

		$('#santa-request').on('input', function () {
...
/* ================================================================ */
/* ========== ФУНКЦИИ ДЛЯ ОБНОВЛЕНИЯ ЦЕНЫ ПОД НАЗВАНИЕМ ========== */
/* ================================================================ */

// Функция для обновления цены под названием товара
function updatePriceUnderName() {
	// Находим все товары в корзине
	var basketItems = document.querySelectorAll('[data-entity="basket-item"]');
	
	basketItems.forEach(function(item) {
		var itemId = item.getAttribute('data-id');
		if (!itemId) return;
		
		// Ищем оригинальные элементы цены (которые мы скрыли)
		var originalSumPrice = item.querySelector('#basket-item-sum-price-' + itemId);
		var originalSumPriceOld = item.querySelector('#basket-item-sum-price-old-' + itemId);
		var originalSumPriceDiff = item.querySelector('#basket-item-sum-price-difference-' + itemId);
		
		// Ищем новые элементы цены под названием
		var totalPriceElement = item.querySelector('#basket-item-total-price-' + itemId);
		var oldTotalPriceElement = item.querySelector('#basket-item-old-total-price-' + itemId);
		var savingsElement = item.querySelector('#basket-item-savings-' + itemId);
		
		// Обновляем общую цену
		if (originalSumPrice && totalPriceElement) {
			totalPriceElement.innerHTML = originalSumPrice.innerHTML;
		}
		
		// Обновляем старую цену (зачеркнутую)
		if (originalSumPriceOld && oldTotalPriceElement) {
			oldTotalPriceElement.innerHTML = originalSumPriceOld.innerHTML;
		}
		
		// Обновляем экономию
		if (originalSumPriceDiff && savingsElement) {
			savingsElement.innerHTML = originalSumPriceDiff.innerHTML;
		}
		
		// Показываем/скрываем блоки со скидкой
		var priceUnderNameBlock = item.querySelector('.basket-item-price-under-name');
		var hasDiscount = originalSumPriceOld && originalSumPriceOld.innerHTML.trim() !== '';
		
		if (priceUnderNameBlock) {
			var savingsBlock = priceUnderNameBlock.querySelector('.basket-item-savings');
			var pricesLineBlock = priceUnderNameBlock.querySelector('.basket-item-prices-line');
			var oldPriceBlock = priceUnderNameBlock.querySelector('.basket-item-old-total-price');
			
			// Показываем/скрываем блоки в зависимости от наличия скидки
			if (savingsBlock) {
				savingsBlock.style.display = hasDiscount ? 'block' : 'none';
			}
			if (pricesLineBlock) {
				pricesLineBlock.style.display = hasDiscount ? 'flex' : 'none';
			}
			if (oldPriceBlock && !hasDiscount) {
				// Если скидки нет, скрываем родительский блок со старой ценой
				var parentPricesLine = oldPriceBlock.closest('.basket-item-prices-line');
				if (parentPricesLine) {
					parentPricesLine.style.display = 'none';
				}
			}
		}
	});
...
// Запускаем функцию обновления после каждого AJAX запроса корзины
var originalRecalcBasketAjax = recalcBasketAjax;
recalcBasketAjax = function(params) {
	var result = originalRecalcBasketAjax.call(this, params);
	
	// Обновляем цены под названием через небольшую задержку
	setTimeout(function() {
		updatePriceUnderName();
	}, 500);
	
	return result;
};

// Обновляем цены при первой загрузке страницы
document.addEventListener('DOMContentLoaded', function() {
	// Проверяем каждые 500ms в течение первых 5 секунд после загрузки
	var attempts = 0;
	var maxAttempts = 10;
...
// Обновляем цены при изменении количества товара
var originalSetQuantity = setQuantity;
setQuantity = function(basketId, ratio, sign, bUseFloatQuantity) {
	var result = originalSetQuantity.call(this, basketId, ratio, sign, bUseFloatQuantity);
