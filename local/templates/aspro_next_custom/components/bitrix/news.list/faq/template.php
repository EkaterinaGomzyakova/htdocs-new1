...
<div class="item-views <?=$arParams['VIEW_TYPE']?> <?=$arParams['VIEW_TYPE']?>-type-block <?=($arParams['SHOW_TABS'] == 'Y' ? 'with_tabs' : '')?> <?=($arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : '')?> <?=($templateName = $component->{'__parent'}->{'__template'}->{'__name'})?>">
	...
	<?endif;?>
		...
		<?endif;?>
				...
				<div class="<?=($arParams['SHOW_TABS'] == 'Y' ? 'tab-content' : 'group-content')?>" itemscope itemtype="https://schema.org/FAQPage">
								...
								<?foreach($arSection['ITEMS'] as $i => $arItem):?>
									...
									<?elseif($arParams['VIEW_TYPE'] == 'accordion'):?>
										<div class="accordion-type-1">
											<div class="item<?=($bImage ? '' : ' wti')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>" itemprop="mainEntity" itemscope itemtype="https://schema.org/Question">
												...
												<div class="accordion-head accordion-close" data-toggle="collapse" data-parent="#accordion<?=$arSection['ID']?>" href="#accordion<?=$arItem['ID']?>_<?=$arSection['ID']?>">
													<span itemprop="name"><?=$arItem['NAME']?><i class="fa fa-angle-down"></i></span>
												...
												<div id="accordion<?=$arItem['ID']?>_<?=$arSection['ID']?>" class="panel-collapse collapse">
													<div class="accordion-body">
														<div class="row">
															<?if(!$bImage):?>
																<div class="col-md-12" itemprop="acceptedAnswer" itemscope itemtype="https://schema.org/Answer">
																	<div class="text" itemprop="text"><?=$textPart?></div>
																</div>
