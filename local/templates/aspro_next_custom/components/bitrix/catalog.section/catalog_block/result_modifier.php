...
if (!empty($arResult['ITEMS'])) {
	...
	foreach ($arResult['ITEMS'] as $key => $arItem) {
		...
		$arItem['WL_PREVIEW_SLIDER'] = [];
		if (is_array($arItem['PROPERTIES']['MORE_PHOTO']['VALUE'])) {
			$arItem['WL_PREVIEW_SLIDER'][] = $arItem['PREVIEW_PICTURE']['SRC'];

			foreach ($arItem['PROPERTIES']['MORE_PHOTO']['VALUE'] as $imageId) {
				$arItem['WL_PREVIEW_SLIDER'][] = CFile::GetPath($imageId);
			}
		}
		...
		$productPictures['PICT'] = CFile::ResizeImageGet(CFile::GetFileArray($productPictures['PICT']['ID']), array('width' => 340, 'height' => 340), BX_RESIZE_IMAGE_PROPORTIONAL_ALT);

		if (!empty($productPictures['PICT'])) {
			$productPictures['PICT'] = array_change_key_case($productPictures['PICT'], CASE_UPPER);
		}
			...
			if ('TYPE_1' == $arParams['TYPE_SKU'] && $arParams['DISPLAY_TYPE'] != 'table') {
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer) {
					...
					$arOneRow = array(
						...
						'AVAILABLE' => CNext::GetQuantityArray($totalCount),
		...
		$dbReviews = CIBlockElement::GetList([], ['IBLOCK_ID' => REVIEWS_IBLOCK_ID, 'PROPERTY_OBJECT' => $arItem['ID'], 'ACTIVE' => "Y"]);
		$arItem['REVIEWS_COUNT'] = $dbReviews->result->num_rows;

		if ($arItem['CODE'] == "podarochnyy_sertifikat") {
			$arSkuIdToCode =
				array_column(
					array_column(
						array_column(
			...
			if (count($arSkuIdToCode)) {
				sort($arSkuIdToCode);
				$defaultPageUrl = $arItem['DETAIL_PAGE_URL'];
				foreach ($arSkuIdToCode as $key => $nominal) {

					$previewPictureVariants =
						array_values(
							array_unique(
								array_column(
									array_column(
					...
					$arItem['PREVIEW_PICTURE']['SRC'] = $previewPictureVariants[$key % (count($previewPictureVariants) - 1)];
					$arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE'] = CurrencyFormat($nominal, 'RUB');
					$arItem['DETAIL_PAGE_URL'] = $defaultPageUrl . '?variant=' . $nominal;
					$arNewItemsList[$key . '-' . $nominal] = $arItem;
				}
			}
		} else {
	...
	$idArray = [];
	$arFilter = array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "SECTION_ID" => $arResult["ID"], "ACTIVE" => "Y", "INCLUDE_SUBSECTIONS" => "Y");
	$res = CIBlockElement::GetList([], $arFilter, false, false, ["ID"]);
	while ($arFields = $res->GetNext()) {
		$idArray[] = $arFields["ID"];
	}

	$arResult["MIN_PRICE"] = PHP_FLOAT_MAX;
	$arResult["MAX_PRICE"] = PHP_FLOAT_MIN;

	$res = CPrice::GetList([], ["PRODUCT_ID" => $idArray, "CATALOG_GROUP_ID" => 1, ">PRICE" => 0]);
	while ($arFields = $res->GetNext()) {
		if ($arFields["PRICE"] < $arResult["MIN_PRICE"]) {
			$arResult["MIN_PRICE"] = $arFields["PRICE"];
		}

		if ($arFields["PRICE"] > $arResult["MAX_PRICE"]) {
			$arResult["MAX_PRICE"] = $arFields["PRICE"];
		}
	}
}
...
$cp = $this->__component;
if (is_object($cp)) {
	$cp->arResult['ITEMS_ID'] = array_column($arResult['ITEMS'], 'ID');
	$cp->SetResultCacheKeys(['ITEMS_ID', 'UF_BACKGROUND_COLOR', 'UF_H1']);
}
