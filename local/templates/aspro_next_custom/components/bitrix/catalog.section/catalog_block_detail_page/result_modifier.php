...
if (!empty($arResult['ITEMS'])) {
    ...
    foreach ($arResult['ITEMS'] as $key => $arItem) {
        ...
        $arItem['PREVIEW_PICTURE'] = CFile::ResizeImageGet(CFile::GetFileArray($arItem['PREVIEW_PICTURE']['ID']), array('width' => 340, 'height' => 340), BX_RESIZE_IMAGE_PROPORTIONAL_ALT);
        $arItem['PREVIEW_PICTURE'] = array_change_key_case($arItem['PREVIEW_PICTURE'], CASE_UPPER);
...
$arLinks = $APPLICATION->IncludeComponent(
	"wl:sticker.colors",
	"",
	Array(
...
$hitValue = $GLOBALS['arrFilterProp']['PROPERTY_HIT_VALUE'][0];
$arHitVariant = CIBlockPropertyEnum::GetList([], ['IBLOCK_ID' => $arParams['IBLOCK_ID'], 'CODE' => 'HIT', 'VALUE' => $hitValue])->Fetch();

if(!empty($arHitVariant['XML_ID'])) {
    $arResult['DETAIL_SECTION_URL'] = $arLinks['ITEMS'][$arHitVariant['XML_ID']]['PROPERTY_LINK_VALUE'];
}

$cp = $this->__component;
if (is_object($cp)) {
    $cp->arResult['ITEMS_ID'] = array_column($arResult['ITEMS'], 'ID');
    $cp->SetResultCacheKeys(array('ITEMS_ID'));
}
