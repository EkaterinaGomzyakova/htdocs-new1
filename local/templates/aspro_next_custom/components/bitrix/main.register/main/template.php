					...
					<form id="registraion-page-form" method="post" action="<?= POST_FORM_ACTION_URI ?>" name="regform"
										...
										<? switch ($FIELD) {
											case "PASSWORD": ?>
												<input size="30" type="password" id="input_<?= $FIELD; ?>"
													name="REGISTER[<?= $FIELD ?>]" required value="<?= $arResult["VALUES"][$FIELD] ?>"
													autocomplete="off"
											...
											<? case "PERSONAL_STREET": ?>
												...
												<? break; ?>
											<? case "EMAIL": ?>
												<input size="30" type="email" id="input_<?= $FIELD; ?>" name="REGISTER[<?= $FIELD ?>]"
													required value="<?= $arResult["VALUES"][$FIELD] ?>" <?= $class ?> id="emails" />
												<? break; ?>
											<? case "NAME": ?>
												<input size="30" type="text" id="input_<?= $FIELD; ?>" name="REGISTER[<?= $FIELD ?>]"
													<?= ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required" : ""); ?>
													value="<?= htmlspecialcharsbx($_REQUEST["REGISTER"]["NAME"]) ?>" <?= $class ?> />
												<? break; ?>
											<? case "PERSONAL_PHONE": ?>
												<input size="30" type="tel" id="input_<?= $FIELD; ?>" name="REGISTER[<?= $FIELD ?>]"
													class="phone_input <?= (array_key_exists($FIELD, $arResult["ERRORS"])) ? 'error' : '' ?>"
													<?= ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required" : ""); ?>
													value="<?= $arResult["VALUES"][$FIELD] ?>" />
												<? break; ?>
											<? case "PERSONAL_BIRTHDAY": ?>
												<input required type="date" id="input_<?= $FIELD; ?>" name="REGISTER[<?= $FIELD ?>]"
													value="<?= $arResult["VALUES"][$FIELD] ?>" />
												<? break; ?>
											<? default: ?>
												<?// hide login ?>
												<input size="30" id="input_<?= $FIELD; ?>" <?= (($FIELD == "LOGIN" && $arTheme["LOGIN_EQUAL_EMAIL"]["VALUE"] == "Y") ? 'type="hidden" value="1"' : 'type="hidden" value="1"'); ?> name="REGISTER[<?= $FIELD ?>]"
													value="<?= $arResult["VALUES"][$FIELD] ?>" />
												<? break; ?>
										<? } ?>
						...
						<? if ($arUFields) { ?>
							<? foreach ($arUFields as $arUField) { ?>
								<? if ($arUField['FIELD_NAME'] == "UF_DENY_SMS") { ?>
									<label>
										<input type="checkbox" value="1" name="<?= $arUField['FIELD_NAME'] ?>">
										<small>Я не даю согласие на получение рекламных материалов и не утверждаю документ <a
												href="/upload/marketing_agreement.pdf" target="_blank">согласие на получение
												рассылок</a></small>
									</label>
								<? } ?>
							<? } ?>
						...
						<? } ?>
						<div class="but-r">
							<p class="register-legals-agreement">Нажимая кнопку "Зарегистрироваться", Вы даете соглашение с
								<a href="/include/licenses_detail.php" target="_blank">политикой обработки персональных
									данных</a> и <a href="/upload/user_agreement.pdf" target="_blank">пользовательским
									соглашением</a>.
							</p>
...
<script>
	$('#input_PERSONAL_PHONE').inputmask("+7 (999) 999-99-99");
</script>