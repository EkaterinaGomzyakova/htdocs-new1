...
<? if ($itemsCnt) { ?>
	<? if ($arParams['AJAX_MODE'] == 'Y' && strpos($_SERVER['REQUEST_URI'], 'bxajaxid') !== false) { ?>
		<script type="text/javascript">
			setTimeout(function() {
				$('.ajax_load .catalog_block .catalog_item_wrapp .catalog_item .item-title').sliceHeight({
					resize: false
				});
				$('.ajax_load .catalog_block .catalog_item_wrapp .catalog_item .cost').sliceHeight({
					resize: false
				});
				$('.ajax_load .catalog_block .catalog_item_wrapp .item_info').sliceHeight({
					resize: false
				});
	...
	<? } ?>
	<div class="right_block1 clearfix catalog <?= strtolower($arTheme["FILTER_VIEW"]["VALUE"]); ?>" id="right_block_ajax" data-design="section-<?= $arResult['VARIABLES']['SECTION_CODE'] ?>">
		...
		<? if ($itemsCnt) { ?>
			<? if ('Y' == $arParams['USE_FILTER']) { ?>
				...
				unset($tmpFilter['FACET_OPTIONS']);
				if($_REQUEST["set_filter"] == "y" || !empty($tmpFilter)) {
					$isFilterSet = true;
				}
				?>
				<div class="adaptive_filter">
					<a class="filter_opener <?= $isFilterSet ? "active" : "" ?>">
						<i></i>
						<span><?= GetMessage("CATALOG_SMART_FILTER_TITLE") ?></span>
					</a>
				</div>
				<script type="text/javascript">
					checkTopFilter();
			...
			<? if ($isAjax == "N") { ?>
				...
				<? $APPLICATION->IncludeComponent(
					...
					array(
						...
						'OFFER_TREE_PROPS' => $arParams['OFFER_TREE_LIST_PROPS'],
