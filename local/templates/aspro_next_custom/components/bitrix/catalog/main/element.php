...
if ($arResult['VARIABLES']['ELEMENT_CODE'] == 'gift-certificate') {
    foreach ($arParams['OFFER_TREE_PROPS'] as $i => $prop) {
        if (!in_array($prop, ['DESIGN', 'NOMINAL', 'GIFT_CERTIFICATE_PRINT'])) {
            unset($arParams['OFFER_TREE_PROPS'][$i]);
        }
    }
}
