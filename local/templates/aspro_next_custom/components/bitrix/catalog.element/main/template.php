...
$arSkuIdCodeMap = [
    'IdToCode' => $arResult['SKU_ID_TO_CODE'] ?? [],
    'CodeToId' => array_flip($arResult['SKU_ID_TO_CODE'] ?? []),
];
$jsobSkuIdCodeMap = \Bitrix\Main\Web\Json::encode($arSkuIdCodeMap);
?>
<script language="JavaScript">
    var skuIdToCodeMap = <?= $jsobSkuIdCodeMap ?>;
</script>
                        ...
                        <td>
                            <? if ('L' == $arResult['PROPERTIES'][$propID]['PROPERTY_TYPE'] && 'C' == $arResult['PROPERTIES'][$propID]['LIST_TYPE']) {
                                foreach ($propInfo['VALUES'] as $valueID => $value) {
                                    ?>
                                    <label>
                                        <input type="radio" name="<? echo $arParams['PRODUCT_PROPS_VARIABLE']; ?>[<? echo $propID; ?>]"
...
if (!empty($arResult['SKU_PROPS'])) {
    if ($arParams['ELEMENT_CODE'] == 'podarochnyy_sertifikat') {
        foreach ($arResult['SKU_PROPS']['DESIGN']['VALUES'] as $i => $arValue) {
            foreach ($arResult['OFFERS'] as $o => $arOffer) {
                if (($arValue['ID'] > 0) && ($arValue['XML_ID'] == $arOffer['PROPERTIES']['DESIGN']['VALUE'])) {
                    $arResult['SKU_PROPS']['DESIGN']['VALUES'][$i]['PICT']['SRC'] = $arResult['OFFERS'][$o]['PREVIEW_PICTURE']['SRC'];
                    break;
                }
            }
        }
...
$arQuantityData = CNext::GetQuantityArray($totalCount, $arItemIDs["ALL_ITEM_IDS"], "N");

$storeItemAmountHtml = function (int $itemId) use ($arItemIDs, $arResult): string {
    $useItemIds = [
        'CATALOG_ITEM_ID' => $arItemIDs['ALL_ITEM_IDS']['CATALOG_ITEM_ID'],
        'STORE_QUANTITY' => 0,
    ];
    $res = '';
    if ($arResult['AMOUNT']['PRODUCTS'][$itemId]['NOT_DEFAULT_STORE_HAS_AMOUNT'] == 'Y') {
        foreach ($arResult['AMOUNT']['PRODUCTS'][$itemId]['AMOUNT_IN_STORE'] as $storeKey => $amount) {
            if ($arResult['AMOUNT']['STORES'][$storeKey]['IS_DEFAULT'] == "Y") {
                $amount -= $arResult['AMOUNT']['PRODUCTS'][$itemId]['AMOUNT_RESERVED'];
                if ($amount <= 0) {
                    continue;
                }
            }
            $quantityHtml = CNext::GetQuantityArray($amount, $useItemIds)['HTML'];

            CModule::IncludeModule("snailshop");

            $quantityHtml = '<div class="store-amount-details-title">'
                . $arResult['AMOUNT']['STORES'][$storeKey]['TITLE']
                . ((SnailShop::userIsStaff()) ? " ($amount)" : '')
                . '</div> <div class="store-amount-details-quantity">'
                . $quantityHtml
                . '</div>';
            $res .= '<div class="store-amount-details">'
                . $quantityHtml
                . '</div>';
        }
        $res = '<div class="store-amount-details-caption"><span>'
            . Loc::getMessage('AVAILABILITY_IN_RETAIL_TITLE')
            . '</span></div>'
            . $res;
    }
    return $res;
...
$useStores = $arParams["USE_STORE"] == "Y"
    && count($arResult['AMOUNT']['STORES'])
    && $arQuantityData["RIGHTS"]["SHOW_QUANTITY"];
...
$without_tags = str_replace(array("</li>", "</ul>"), ". ", $arResult['DETAIL_TEXT']);
                    ...
                    <div class="offers_img wof">
                        ...
                        <? if ($arFirstPhoto["BIG"]["src"]) { ?>
                            <a href="<?= ($viewImgType == "POPUP" ? $arFirstPhoto["BIG"]["src"] : "javascript:void(0)"); ?>"
                               class="<?= ($viewImgType == "POPUP" ? "popup_link" : "line_link"); ?>" title="<?= $title; ?>">
                            ...
                            <? foreach ($arResult["MORE_PHOTO"] as $i => $arImage) {
                                switch ($arImage["TYPE"]) {
                                    case "image":
                                        ...
                                        $title = $arImage["TITLE"];
                                        ?>
                                        <li id="photo-<?= $i ?>" class="slide-image<?= (!$i ? ' current' : '') ?>"<?= ($i ? ' style="display: none;"' : '') ?>>
                                            <? if (!$isEmpty) {
                                                ?>
                                                <a href="<?= ($viewImgType == "POPUP" ? $arImage["BIG"]["src"] : "javascript:void(0)"); ?>" <?= ($bIsOneImage ? '' : 'data-fancybox-group="item_slider"') ?>
                                                   class="<?= ($viewImgType == "POPUP" ? "popup_link fancy" : "line_link"); ?>" title="<?= $title; ?>">
                                                    <img src="<?= $arImage["BIG"]["src"] ?>" <?= ($viewImgType == "MAGNIFIER" ? "class='zoom_picture'" : ""); ?> <?= ($viewImgType == "MAGNIFIER" ? 'xoriginal="' . $arImage["BIG"]["src"] . '" xpreview="' . $arImage["THUMB"]["src"] . '"' : ""); ?>
	                                ...
	                                case "video_youtube":
		                                ?>
                                        <li id="photo-<?= $i ?>" class="slide-youtube<?= (!$i ? ' current' : '') ?>"<?= ($i ? ' style="display: none;"' : '') ?>>
                                            <iframe
                                                    id="youtube_<?=$arImage["CODE"]?>"
                                                    width="100%"
                                                    height="100%"
                                                    src="<?=$arImage["SRC_IFRAME"]?>"
                                                    title="Video player"
                                                    frameborder="0"
	                                ...
	                                case "video":
		                                ?>
                                        <li id="photo-<?= $i ?>" class="slide-video<?= (!$i ? ' current' : '') ?>"<?= ($i ? ' style="display: none;"' : '') ?>>
                                            <video
                                                    src="<?=$arImage["URL"]?>"
                                                    controls
                                                    playsinline>
                                                <source src="<?=$arImage["URL"]?>" type="video/mp4">
            ...
            if (count($arResult["MORE_PHOTO"]) > 1):?>
                <div class="wrapp_thumbs xzoom-thumbs">
                    <div class="thumbs flexslider"
                         data-plugin-options='{"animation": "slide", "selector": ".slides_block > li", "directionNav": true, "itemMargin":10, "itemWidth": 54, "controlsContainer": ".thumbs_navigation", "controlNav" :false, "animationLoop": true, "touch": true, "slideshow": false}'
                                ...
                                <? switch ($arImage["TYPE"]) {
                                    ...
                                    ?>
                                        <li <?= (!$i ? 'class="current"' : '') ?> data-big_img="<?= $arImage["BIG"]["src"] ?>" data-small_img="<?= $arImage["SMALL"]["src"] ?>">
                                            <span><img class="xzoom-gallery" width="50" xpreview="<?= $arImage["THUMB"]["src"]; ?>" src="<?= $arImage["THUMB"]["src"] ?>"
                                                       alt="<?= $arImage["ALT"]; ?>" title="<?= $arImage["TITLE"]; ?>"/></span>
                                        </li>
                                    <? break;
                                    case "video_youtube":
                                        ?>
                                        <li <?= (!$i ? 'class="current"' : '') ?>>
                                            <span><img class="xzoom-gallery" width="50" xpreview="<?= $arImage["SRC_THUMBNAIL"]; ?>" src="<?= $arImage["SRC_THUMBNAIL"] ?>"
                                               alt="Видео Youtube" title="Видео Youtube"/></span>
                                        </li>
                                    <? break;
                                    case "video":
                                        ?>
                                        <li <?= (!$i ? 'class="current"' : '') ?>>
                                            <span><img class="xzoom-gallery" width="50" xpreview="" src="<?=$templateFolder?>/images/icon_video.svg"
                                                       alt="Просмотр видео" title="Просмотр видео"/></span>
                                        </li>
                                <?  break;
                                } ?>
                            <? } ?>
...
<div class="tabs_section">
        ...
        <? if (!$showCustomOffer || empty($arResult['OFFERS_PROP'])) { ?>
            <div class="item_slider color-controls flex flexslider"
                 data-plugin-options='{"animation": "slide", "directionNav": false, "controlNav": true, "animationLoop": false, "slideshow": false, "slideshowSpeed": 10000, "touch": true, "animationSpeed": 600}'>
                <ul class="slides">
                    <? if ($arResult["MORE_PHOTO"]) {
                        foreach ($arResult["MORE_PHOTO"] as $i => $arImage) {
	                        switch ($arImage["TYPE"]) {
		                        case "image":
			                        ...
			                        $title = $arImage["TITLE"];
			                        ?>
                                    <li id="mphoto-<?= $i ?>" class="slide-image<?= (!$i ? ' current' : '') ?>"<?= ($i ? ' style="display: none;"' : '') ?>>
				                        <? if (!$isEmpty) {
					                        ?>
		                        ...
		                        case "video_youtube":
			                        ?>
                                    <li id="mphoto-<?= $i ?>" class="slide-youtube<?= (!$i ? ' current' : '') ?>"<?= ($i ? ' style="display: none;"' : '') ?>>
                                        <iframe
                                                id="youtube_<?=$arImage["CODE"]?>"
                                                width="100%"
                                                height="100%"
		                        ...
		                        case "video":
			                        ?>
                                    <li id="mphoto-<?= $i ?>" class="slide-video<?= (!$i ? ' current' : '') ?>"<?= ($i ? ' style="display: none;"' : '') ?>>
                                        <video src="<?=$arImage["URL"]?>"
                                               controls
                                               playsinline>
                                            <source src="<?=$arImage["URL"]?>" type="video/mp4">
                                            Sorry, your browser doesn't support embedded videos,
                                            but don't worry, you can <a href="<?=$arImage["URL"]?>">download it</a>
    ...
    <div class="right_info" style="width: 380px; margin-left: auto;">
        <div class="info_item">
            <? $isArticle = (strlen($arResult["DISPLAY_PROPERTIES"]["CML2_ARTICLE"]["VALUE"]) || ($arResult['SHOW_OFFERS_PROPS'] && $showCustomOffer)); ?>
            
            <div class="element_subtitle"><?= $arResult['PROPERTIES']['ALT_NAME']['VALUE'] ?></div>
                ...
                <? } ?>
                    ...
                    <div class="buy_block">
                        ...
                        <? if (!$arResult["OFFERS"]): ?>
                            ...
                            <div class="counter_wrapp">
                                <? if (($arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] && $arAddToBasketData["ACTION"] == "ADD") && $arAddToBasketData["CAN_BUY"]): ?>
                                    <div class="counter_block big_basket" data-offers="<?= ($arResult["OFFERS"] ? "Y" : "N"); ?>"
                                        data-item="<?= $arResult["ID"]; ?>" <?= (($arResult["OFFERS"] && $arParams["TYPE_SKU"] == "N") ? "style='display: none;'" : ""); ?>>
                                        <span class="minus" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['QUANTITY_DOWN']; ?>">-</span>
                                        <input type="text" class="text" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['QUANTITY']; ?>" name="<? echo $arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>"
                                ...
                                <div id="<? echo $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>"
                                    ...
                                    <?if(!$arAddToBasketData["CAN_BUY"]):?>
                                        <div class="email-subscribe-description">Уведомление придет на ваш E-Mail</div>
                                    <?endif;?>
                                </div>
                            </div>
                        <? if (isset($arResult['PRICE_MATRIX']) && $arResult['PRICE_MATRIX']) // USE_PRICE_COUNT
                        ...
                        <? if ($arAddToBasketData["ACTION"] == "ADD" && $arAddToBasketData["CAN_BUY"] && $arParams["SHOW_ONE_CLICK_BUY"] != "N"): ?>
                            <div class="wrapp_one_click">
                                            <span class="btn btn-default white btn-lg type_block transition_bg one_click" data-item="<?= $arResult["ID"] ?>"
                                                data-iblockID="<?= $arParams["IBLOCK_ID"] ?>" data-quantity="<?= $arAddToBasketData["MIN_QUANTITY_BUY"]; ?>"
                        ...
                        <div class="like_wrapper">
                            <div class="like_icons iblock">
                                    ...
                                    <? if (!$arResult["OFFERS"]): ?>
                                        ...
                                        <?else:?>
                                            <div class="wish_item text" >
                                                <span class="value" data-toggle="tooltip" title="<?= GetMessage('TOOLTIP_WISHIST') ?>"><i></i></span>
                                            </div>
                                        <? endif; ?>
                                    <? elseif ($arResult["OFFERS"] && $arParams["TYPE_SKU"] === 'TYPE_1' && !empty($arResult['OFFERS_PROP'])): ?>
                                        <div class="wish_item text " <?= ($arAddToBasketData['CAN_BUY'] ? '' : 'style="display:none"'); ?> data-item=""
                                ...
                                <? if ($arParams["DISPLAY_COMPARE"] == "Y"): ?>
                                    <? if (!$arResult["OFFERS"]): ?>
                                        <div data-item="<?= $arResult["ID"] ?>" data-iblock="<?= $arResult["IBLOCK_ID"] ?>" data-href="<?= $arResult["COMPARE_URL"] ?>"
                                    ...
                                    <? elseif ($arResult["OFFERS"] && $arParams["TYPE_SKU"] === 'TYPE_1'): ?>
                                        <div data-item="" data-iblock="<?= $arResult["IBLOCK_ID"] ?>" data-href="<?= $arResult["COMPARE_URL"] ?>"
                    ...
                    <? if ($arResult["COUNT_PRODUCT_IN_BASKET"]) { ?>
                        <div class="count-product-baskets-wrapper">
                            ...
                            <?
                            $countDeclension = new Declension('человека', 'человек', 'человек');
                            $declension = $countDeclension->get($arResult["COUNT_PRODUCT_IN_BASKET"]);
                            ?>
                                    ...
                                    <? foreach ($arSetItem["PRICES"] as $key => $arPrice): ?>
                                        <? if ($arPrice["CAN_ACCESS"]): ?>
                                            ...
                                            <? if ($arPrice["VALUE"] > $arPrice["DISCOUNT_VALUE"] && $arParams["SHOW_OLD_PRICE"] == "Y"): ?>
                                                <div class="price">
                                                    <?= $arPrice["PRINT_DISCOUNT_VALUE"]; ?><? if (($arParams["SHOW_MEASURE"] == "Y") && $strMeasure): ?><small>
                                                        /<?= $strMeasure ?></small><? endif; ?>
                                                </div>
                                            ...
                                            <? else: ?>
                                                <div class="price">
                                                    <?= $arPrice["PRINT_VALUE"]; ?><? if (($arParams["SHOW_MEASURE"] == "Y") && $strMeasure): ?><small>
                                                        /<?= $strMeasure ?></small><? endif; ?>
                                                </div>
                                            <? endif; ?>
                                        <? endif; ?>
...
<? if ($arResult['OFFERS']): ?>
    ...
    <? endif; ?>
<? else: ?>
    <? $APPLICATION->IncludeComponent("bitrix:catalog.set.constructor", "bootstrap_v4",
        array(
...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="product-detail-feedback">
            ...
            <p class="heading1">Нужна консультация?</p>
            <p class="heading2">Наши специалисты ответят на любой вопрос</p>
            <div class="messengers">
                <a target="_blank" href="https://t.me/clanbeauty" class="messenger-icon messenger-ico-tg"></a>
                <a target="_blank" href="https://api.whatsapp.com/send?phone=79202488898" class="messenger-icon messenger-ico-wa"></a>
            </div>
        </div>
...
<?php if (!empty($arResult['SKU_PROPS'])) {
    $skuPropId = $arResult['SKU_PROPS']['DESIGN']['ID'];
    $skuPropValueId = 0;
    foreach ($arResult['SKU_PROPS']['DESIGN']['VALUES'] as $arValue) {
        if ($arValue['ID'] > 0) {
            $skuPropValueId = $arValue['ID'];
            break;
        }
    }

    $treeValueElement = $skuPropId . '_' . $skuPropValueId;

    $request = Context::getCurrent()->getRequest();

    $variantPropId = $arResult['SKU_PROPS']['NOMINAL']['ID'];
    $variantId = 0;

    foreach ($arResult['SKU_PROPS']['NOMINAL']['VALUES'] as $i => $arValue) {
        if (($arValue['ID'] > 0) && ($arValue['NAME'] == $request->get('variant'))) {
            $variantId = $arValue['ID'];
        }
    }

    $propVariantTreeValue = $variantPropId . '_' . $variantId;

    if ($arParams['ELEMENT_CODE'] == 'podarochnyy_sertifikat') {?>
        <script>
            BX.ready(function() {
                const firstElement = document.querySelector('.bx_scu_scroller_container li[data-treevalue="<?=$treeValueElement?>"]')

                if (firstElement != null) {
                    firstElement.click();
                }

                const variantElement = document.querySelector('li[data-treevalue="<?=$propVariantTreeValue?>"]')
