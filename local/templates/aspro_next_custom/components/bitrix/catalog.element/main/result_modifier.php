...
// get array relation for SKU ID and CODE
$arResult['SKU_ID_TO_CODE'] = [];
$arSkuIdToCode = array_column(
	array_map(
		'current',
		array_column(
			array_column(
...
if (count($arSkuIdToCode)) {
	$arResult['SKU_ID_TO_CODE'] = $arSkuIdToCode;
}

if (empty($arResult['SKU_ID_TO_CODE'])) {
	$arSkuIdToCode =
	array_column(
		array_column(
				array_column(
...
if (file_exists($_SERVER['DOCUMENT_ROOT'] . $strEmptyPreview)) {
	...
	if (!empty($arSizes)) {
		$arEmptyPreview = [
			'BIG' => $strEmptyPreview,
			'SMALL' => $strEmptyPreview,
			'THUMB' => $strEmptyPreview,
		];
	}
	unset($arSizes);
}
...
$arResult['CATALOG_QUANTITY'] = (0 < $arResult['CATALOG_QUANTITY'] && is_float($arResult['CATALOG_MEASURE_RATIO'])
...
if ($arResult['PREVIEW_PICTURE'] && 'Y' == $arParams['ADD_DETAIL_TO_SLIDER'] && count($arResult['OFFERS']) == 0) {
	$productSlider = CNext::getSliderForItemExt($arResult, $arParams['ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER']);
} else if (count($arResult['OFFERS']) > 0 && strlen(current($arResult['OFFERS'])['PREVIEW_PICTURE']['SRC']) > 0) {
	$arOffer = current($arResult['OFFERS']);
	$productSlider[] = [
		'BIG' => $arOffer['PREVIEW_PICTURE']['SRC'],
		'SMALL' => $arOffer['PREVIEW_PICTURE']['SRC'],
		'THUMB' => $arOffer['PREVIEW_PICTURE']['SRC'],
	];
} else {
	$productSlider = [$arEmptyPreview];
}
...
if ($productSlider) {
	foreach ($productSlider as $i => $arImage) {
		...
		if (!empty($arImage["ID"])) {
			$productSlider[$i] = array_merge(
				...
				array(
					"TYPE" => "image",
                    "BIG" => CFile::ResizeImageGet($arImage["ID"], array("width" => 694, "height" => 748), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
                    "SMALL" => CFile::ResizeImageGet($arImage["ID"], array("width" => 694, "height" => 748), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
			...
			$productSlider[$i] = array(
				"TYPE" => "image",
				"BIG" => ['src' => $arImage['BIG']],
				"SMALL" => ['src' => $arImage['SMALL']],
				"THUMB" => ['src' => $arImage['THUMB']],
			);
		}
	}
	if (!empty($arResult['PROPERTIES']['VIDEO_YOUTUBE']['VALUE'])) {
		if (preg_match("~^((?:https?:)?\/\/)?((?:www|m)\.)?((?:youtube(-nocookie)?\.com|youtu.be))(\/(?:[\w\-]+\?v=|embed\/|v\/)?)([\w\-]+)(\S+)?$~", $arResult['PROPERTIES']['VIDEO_YOUTUBE']['VALUE'], $matches)) {
			$videoCode = $matches[6];
			$productSlider[] = [
				"TYPE" => "video_youtube",
				"URL" => $arResult['PROPERTIES']['VIDEO_YOUTUBE']['VALUE'],
				"CODE" => $videoCode,
				"SRC_IFRAME" => "https://www.youtube-nocookie.com/embed/$videoCode?controls=0&enablejsapi=1&playsinline=1",
				"SRC_ORIGINAL" => "https://i.ytimg.com/vi/$videoCode/maxresdefault.jpg",
				"SRC_THUMBNAIL" => "https://i.ytimg.com/vi/$videoCode/1.jpg",
			];
		}
	}
	if (!empty($arResult['PROPERTIES']['VIDEO']['VALUE'])) {
		$productSlider[] = [
			...
			"URL" => CFile::GetPath($arResult['PROPERTIES']['VIDEO']['VALUE']),
		];
	}
}
			...
			if ($arOffer['PROPERTIES'] && isset($arOffer['PROPERTIES'][$arParams['ADDITIONAL_GALLERY_OFFERS_PROPERTY_CODE']]) && $arOffer['PROPERTIES'][$arParams['ADDITIONAL_GALLERY_OFFERS_PROPERTY_CODE']]['VALUE']) {
				foreach ($arOffer['PROPERTIES'][$arParams['ADDITIONAL_GALLERY_OFFERS_PROPERTY_CODE']]['VALUE'] as $img) {
					$arPhoto = array(
                        ...
                        'PREVIEW' => CFile::ResizeImageGet($img, array('width' => 694, 'height' => 748), BX_RESIZE_PROPORTIONAL_ALT, true),
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])) {
	...
	if ('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS']) {
		foreach ($arResult['OFFERS'] as &$arOffer) {
			foreach ($arSKUPropIDs as &$strOneCode) {
				if (isset($arOffer['DISPLAY_PROPERTIES'][$strOneCode])) {
					...
					$valueId = ($arSKUPropList[$strOneCode]['PROPERTY_TYPE'] == Iblock\PropertyTable::TYPE_LIST
		...
		// add CODE to props
		foreach ($arSKUPropList['VARIANT']['VALUES'] as $skuId => &$skuPropertyValue) {
			$skuPropertyValue['CODE'] = $arSkuIdToCode[$skuId];
		}
		...
		if ($arOffer['MORE_PHOTO']) {
			foreach ($arOffer['MORE_PHOTO'] as $i => $arImage) {
				if ($arImage["ID"]) {
                    ...
                    $arOffer['MORE_PHOTO'][$i]["SMALL"] = CFile::ResizeImageGet($arImage["ID"], array("width" => 694, "height" => 748), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
	...
	if ('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS']) {
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer) {
			...
			$arOneRow = array(
				...
				'AVAILABLE' => CNext::GetQuantityArray($totalCount, array(), "Y"),
...
foreach ($arResult['OFFERS'] as $key1 => $arOffer) {
	foreach ($arOffer['DISPLAY_PROPERTIES'] as $key2 => $displayProp) {
		foreach ($arSKUPropList as $key3 => $arProp) {
			if ($displayProp['ID'] == $arProp['ID']) {
				$id = $displayProp['VALUE'];
				$arSKUPropList[$key3]['VALUES'][$id]['AVAILABLE'] = $arOffer['PRODUCT']['AVAILABLE'];
			}
		}
	}
}
...
// https://www.php.net/manual/ru/function.array-multisort.php#100534
...
foreach ($arSKUPropList as $key => $arProp) {
	if (is_array($arSKUPropList[$key]['VALUES'])) {
		$arSKUPropList[$key]['VALUES'] = array_orderby($arSKUPropList[$key]['VALUES'], 'AVAILABLE', SORT_DESC, 'NAME', SORT_ASC);
	}
}
...
if (is_array($arResult['DISPLAY_PROPERTIES']['BRAND']['LINK_ELEMENT_VALUE'])) {
	$brand = current($arResult['DISPLAY_PROPERTIES']['BRAND']['LINK_ELEMENT_VALUE']);
	$arResult['DISPLAY_PROPERTIES']['BRAND']['DISPLAY_VALUE'] = '<a href="' . $brand['DETAIL_PAGE_URL'] . '">' . $brand['NAME'] . '<br><span class="small pops_list_brand_link">' . GetMessage('ALL_BRAND_PRODUCTS_TEXT') . '</span></a>';
}
...
$arResult['COUNT_EARN_POINTS'] = ceil($arResult['MIN_PRICE']['DISCOUNT_VALUE'] / 100 * COUNT_BONUS_POINTS);
$pointsDeclension = new \Bitrix\Main\Grid\Declension('бьюти-балл', 'бьюти-балла', 'бьюти-баллов');
$arResult['EARN_POINT_TEXT'] = $pointsDeclension->get($arResult['COUNT_EARN_POINTS']);

$ipropValues = new \Bitrix\Iblock\InheritedProperty\ElementValues($arResult['IBLOCK_ID'], $arResult['ID']);
$arResult['SEO_PROPERTIES']  = $ipropValues->getValues();

CModule::IncludeModule("parnas.khayrcomment");
$arResult['REVIEWS_COUNT'] = 0;
$dbReviewCount = CIBlockElement::GetList([], ['IBLOCK_ID' => REVIEWS_IBLOCK_ID, 'ACTIVE' => "Y", 'PROPERTY_OBJECT' => $arResult['ID']], false, false, ['ID']);
while ($arReview = $dbReviewCount->Fetch()) {
	$arResult['REVIEWS_COUNT']++;
}

if (!empty($arResult['JS_OFFERS'])) {
	foreach ($arResult['JS_OFFERS'] as $keyOffer => &$jsOffer) {
		$originalOffer = $arResult['OFFERS'][$keyOffer];
		$jsOffer['PREVIEW_TEXT'] = $originalOffer['PREVIEW_TEXT'];
	}
	unset($jsOffer);
}

$arResult["COUNT_PRODUCT_IN_BASKET"] = 0;

if ($arResult["PRODUCT"]["QUANTITY"] > 0) {
	$arResult["COUNT_PRODUCT_IN_BASKET"] = WL\Basket::countProductInBaskets($arResult["ID"]);
}


//// + Determine availability of products in stores, including SKU
...
while ($arStore = $dbStores->Fetch()) {
	$arResult['AMOUNT']['STORES'][$arStore['ID']] = $arStore;
	if ($arStore['IS_DEFAULT'] == 'Y') {
		$defaultStore = $arStore['ID'];
	}
}

// make common list of product and SKUs
$itemIds = [$arResult['ID']];
...
// fetch quantity reserved for common product list
...
while ($arCatalogProduct = $dbCatalogProduct->Fetch()) {
	$catalogProduct[$arCatalogProduct['ID']] = $arCatalogProduct['QUANTITY_RESERVED'];
}

// fetch storage amounts for common product list
$dbStoreProduct = CCatalogStoreProduct::GetList(
	[],
	[
		'PRODUCT_ID' => $itemIds,
		'STORE_ID' => array_keys($arResult['AMOUNT']['STORES']),
	],
	false,
	false,
...
while ($arStoreProduct = $dbStoreProduct->Fetch()) {
	if (!isset($arResult['AMOUNT']['PRODUCTS'][$arStoreProduct['PRODUCT_ID']])) {
		$arResult['AMOUNT']['PRODUCTS'][$arStoreProduct['PRODUCT_ID']] = [
			'IS_SKU' => $arStoreProduct['PRODUCT_ID'] == $arResult['ID'] ? 'N' : 'Y',
			'NOT_DEFAULT_STORE_HAS_AMOUNT' => 'N',
			'AMOUNT_RESERVED' => $catalogProduct[$arStoreProduct['PRODUCT_ID']],
			'AMOUNT_IN_STORE' => []
		];
	}
	$arResult['AMOUNT']['PRODUCTS'][$arStoreProduct['PRODUCT_ID']]['AMOUNT_IN_STORE'][$arStoreProduct['STORE_ID']] = $arStoreProduct['AMOUNT'];
	if ($arStoreProduct['STORE_ID'] != $defaultStore && $arStoreProduct['AMOUNT'] > 0) {
		$arResult['AMOUNT']['PRODUCTS'][$arStoreProduct['PRODUCT_ID']]['NOT_DEFAULT_STORE_HAS_AMOUNT'] = 'Y';
	}
}
//// ~ Determine availability of products in stores, including SKU