...
$(document).ready(function () {
	...
	BX.addCustomEvent('onCompleteAction', function (eventdata, _this) {
		try {
			if (eventdata.action === 'loadForm') {
				...
				$('.mobile-app-menu .icon-basket .count').text(arBasketPrices.BASKET_COUNT);
	...
	$(document).on('click', '.to-subscribe', function (e) {
		...
		if (!th.hasClass('clicked')) {
			...
			if ($(this).is('.auth')) {
				...
				let $this = $(this);
				$.get('/ajax/user.php', { action: 'check_email' }, function (response) {
	...
	BX.addCustomEvent('onSelectFlexsliderThumb', function (eventdata) {
		var test = $(eventdata.data.currentTarget);
		if (!test.hasClass("current")) {
			var slider = test.parents(".item_slider").find(".slides").first();
			var targetIndex = test.parent().children().index(test) + 1;
			var targetSlide = slider.find("li:nth-child(" + targetIndex + ")");

			targetSlide.siblings(".slide-video").each(function () {
			...
			if (targetSlide.hasClass("slide-video")) {
	...
	$("#thumbs li").click(function (e) {
		BX.onCustomEvent('onSelectFlexsliderThumb', e);
	});
});
