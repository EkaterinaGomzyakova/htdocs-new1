	...
	foreach ($arResult['PRODUCTS'] as $arItems)
	{
		...
		if (!empty($arItems['SKU_ITEMS']) && !empty($arItems['SKU_ITEMS']["SKU_ELEMENTS"]))
		{
			...
			foreach ($arSkuResult["SKU_ELEMENTS"] as $val)
			{
                ...
                $rowSku->AddViewField("RESERVED", $val['QUANTITY_RESERVED']);

                if (!empty($val['AMOUNT'])) {
                    foreach ($val['AMOUNT'] as $s => $arStoreAmount) {
                        $rowSku->AddViewField('STORE' . $s, $arStoreAmount['QUANTITY']);
                        if ($arStoreAmount['IS_DEFAULT'] == 'Y' && $arStoreAmount['STORE_ID'] == $arResult['USER_STORE_ID']) {
                            if ($arStoreAmount['QUANTITY'] - $arItems['PRODUCT']['QUANTITY_RESERVED'] <= 0) {
                                $arSkuActions[] = array(
                                    "TEXT" => GetMessage("SPS_SELECT"),
                                    "DEFAULT" => "Y",
                                    "ACTION" => 'return false;'
                                ...
                                $rowSku->AddField("ACTION", '<span class="select-sku-text">' . GetMessage('PRODUCT_FIELD_RESERVE_WORD') . '</span>');
                            } else {
                                $arSkuActions[] = array(
                                    "TEXT" => GetMessage("SPS_SELECT"),
                                    "DEFAULT" => "Y",
		...
		else
		{
			...
			if (!$isSection)
			{
                ...
                if (!empty($arItems['PRODUCT']['AMOUNT'])) {
                    foreach ($arItems['PRODUCT']['AMOUNT'] as $s => $arStoreAmount) {
                        if ($arStoreAmount['IS_DEFAULT'] == 'Y' && $arStoreAmount['STORE_ID'] == $arResult['USER_STORE_ID']) {
                            if ($arStoreAmount['QUANTITY'] - $arItems['PRODUCT']['QUANTITY_RESERVED'] <= 0) {
                                $arActions[] = array(
                                    "TEXT" => GetMessage("SPS_SELECT"),
                                ...
                                $row->AddField("ACTION", '<span class="select-sku-text">' . GetMessage('PRODUCT_FIELD_RESERVE_WORD') . '</span>');
                            } else {
                                $arActions[] = array(
                                    "TEXT" => GetMessage("SPS_SELECT"),
                                    "DEFAULT" => "Y",
            ...
            if (!empty($arItems['PRODUCT']['AMOUNT'])) {
                foreach ($arItems['PRODUCT']['AMOUNT'] as $s => $arStoreAmount) {
                    $row->AddViewField("STORE" . $s, $arStoreAmount['QUANTITY']);
                }
            }

            $row->AddViewField("RESERVED", $arItems['PRODUCT']['QUANTITY_RESERVED']);
		}
