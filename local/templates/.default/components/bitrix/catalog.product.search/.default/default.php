...
else
{
    ...
    // список ID товаров на складах
    $productIds = [array_keys($arResult['PRODUCTS'])];
    // добавляем список ID SKU на складах
    foreach ($arResult['PRODUCTS'] as $arProduct) {
        if (isset($arProduct['SKU_ITEMS'])) {
            $productIds[] = $arProduct['SKU_ITEMS']['SKU_ELEMENTS_ID'];
        }
    }
    ...
    while($arStoreProduct = $rsStoreProduct->fetch()) {
        if ((int) $arStoreProduct['AMOUNT'] > 0) {
            $pid = (int) $arStoreProduct['PRODUCT_ID'];
            $sid = (int) $arStoreProduct['STORE_ID'];
            if (!isset($arStoreProducts[$pid])) {
                $arStoreProducts[$pid] = [];
            }
            $arStoreProducts[$pid][$sid] = [
                'ID' => (int) $arStoreProduct['ID'],
                'STORE_ID' => $sid,
                'AMOUNT' => (int) $arStoreProduct['AMOUNT'],
                'RESERVED' => (int) $arStoreProduct['QUANTITY_RESERVED'],
            ];
        }
    }
	...
	foreach ($arResult['PRODUCTS'] as $arItems)
	{
		...
		if (!empty($arItems['SKU_ITEMS']) && !empty($arItems['SKU_ITEMS']["SKU_ELEMENTS"]))
		{
			...
			foreach ($arSkuResult["SKU_ELEMENTS"] as $val)
			{
                ...
                if ($balance > 0 && isset($arStoreProducts[$val['ID']])) {
                    $balance = array_map(static function ($e) use ($arStores) {
                        $result = "<nobr><small style=\"color: gray;\">{$arStores[$e['STORE_ID']]['TITLE']}:</small> {$e['AMOUNT']}";
                        if ($e['RESERVED']) {
                            $result .= "<br><small>резерв: {$e['RESERVED']}</small>";
                        }
                        $result .= "</nobr>";
                        return $result;
                    }, $arStoreProducts[$val['ID']]);
		...
		else
		{
			...
			{
                $balance = 0;
                if ($arCatalogProduct['QUANTITY'] > 0 && isset($arStoreProducts[$arCatalogProduct['ID']])) {
                    $balance = array_map(static function ($e) use ($arStores) {
                        $result = "<nobr><small style=\"color: gray;\">{$arStores[$e['STORE_ID']]['TITLE']}:</small> {$e['AMOUNT']}";
                        if ($e['RESERVED']) {
                            $result .= "<br><small>резерв: {$e['RESERVED']}</small>";
                        ...
                        $result .= "</nobr>";
                        return $result;
                    }, $arStoreProducts[$arCatalogProduct['ID']]);
                    $balance = implode('<br>', $balance);
                }
			}
