class Roulete {
	...
	pieSeries = {};
	signedParameters = '';
	agreementCheckbox = document.querySelector('.roulete-form #roulete_agreement');
	...
	roll() {
		if (!this.agreementCheckbox.checkValidity()) {
			this.agreementCheckbox.reportValidity();
			return false;
		}
		...
		document.querySelector('#agreement-form').style.display = "none";
		document.querySelector('#order-id').style.display = "none";


		BX.ajax.runComponentAction("clanbeauty:roulete", 'getRouleteResult', {
			mode: 'class',
			...
			if (response.data.id) {
				...
				window.setTimeout(() => {
					...
					if (parseInt(response.data.ordersLeft) > 0) {
						document.querySelector('.again').style.display = "block";

						document.querySelector('.roulete-order-count').innerHTML = response.data.ordersLeftText;
						document.querySelector('.roulete-order-count').style.display = "block";
					}
				}, 6000);
			} else {
				this.showMessage(response.data.status, response.data.message)
	...
	initChart(prizes) {
		...
		this.chart.events.on('ready', () => {
			this.pieSeries.labels.each((el) => {
				...
				el.html = `<p style="color:${el.dataItem.dataContext.text_color};">${el.dataItem.dataContext.name}</p><img src='${el.dataItem.dataContext.image}'/>`;
			});
			this.pieSeries.reinit();
