...
if($_SERVER["REQUEST_METHOD"]=="POST" && ($_REQUEST["save"] <> '' || $_REQUEST["apply"] <> '') && check_bitrix_sessid())
{
	...
	if($strError == '')
	{
        ...
        /**
         * День рождения можно изменить только один раз
         */
        if (!empty($arFields["PERSONAL_BIRTHDAY"])) {
            if ($arUser && empty($arUser["PERSONAL_BIRTHDAY"])) {
                $arFields["PERSONAL_BIRTHDAY"] = \CDatabase::FormatDate($arFields["PERSONAL_BIRTHDAY"], "YYYY-MM-DD", "DD.MM.YYYY");
            } else {
