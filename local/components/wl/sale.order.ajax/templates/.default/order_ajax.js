...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        initFirstSection: function () {
            ...
            // Обеспечиваем, что блоки покупателя, доставки и оплаты всегда раскрыты
            this.ensureAlwaysExpandedBlocks();
        },

        ensureAlwaysExpandedBlocks: function () {
            var alwaysExpandedBlocks = ['bx-soa-delivery', 'bx-soa-paysystem'];
            var self = this;
            
            alwaysExpandedBlocks.forEach(function(blockId) {
                var blockNode = BX(blockId);
                if (blockNode) {
                    // Убеждаемся, что блок активен и выбран
                    BX.addClass(blockNode, 'bx-active');
                    BX.addClass(blockNode, 'bx-selected');
                    
                    // Принудительно отображаем блок и его контент
                    blockNode.style.display = 'block';
                    var contentNode = blockNode.querySelector('.bx-soa-section-content');
                    if (contentNode) {
                    ...
                    // Кнопки редактирования уже скрыты через CSS
                    
                    // Убираем обработчики клика для заголовков
                    var titleContainer = blockNode.querySelector('.bx-soa-section-title-container');
                    if (titleContainer) {
                        BX.unbindAll(titleContainer);
                    }
                }
            });
            
            // Специальная обработка для блока покупателя
            var buyerContainer = BX('bx-soa-buyer-inside-delivery');
            if (buyerContainer) {
                buyerContainer.style.display = 'block';
                var buyerContent = buyerContainer.querySelector('.bx-soa-section-content');
                if (buyerContent) {
                    buyerContent.style.display = 'block';
                    buyerContent.style.opacity = '1';
                    buyerContent.style.visibility = 'visible';
                }
            }
        },
        ...
        getBlockFooter: function (node) {
            // Отключаем создание кнопок навигации
            var buttons = [];
        ...
        editSection: function (section) {
            ...
            // Проверяем, является ли блок всегда раскрытым
            var alwaysExpandedBlocks = ['bx-soa-properties', 'bx-soa-delivery', 'bx-soa-paysystem'];
            var isAlwaysExpanded = alwaysExpandedBlocks.indexOf(section.id) !== -1;
            ...
            // Для всегда раскрытых блоков принудительно делаем active = true
            if (isAlwaysExpanded) {
                active = true;
            }
            ...
            // Отключаем все обработчики клика для заголовков и кнопок редактирования
            // (кнопки навигации убраны)
        ...
        getDeliveryLocationInput: function (node) {
            var currentProperty, locationId, altId, location, k, altProperty,
                ...
                labelTextHtml, label, input, altNode, quickLocationsLabel;
            ...
            if (location && location[0] && location[0].output) {
                ...
                /*quickLocationsLabel = '<span class="bx-ui-sls-quick-locations-label">'+ BX.message('SOA_POPULAR_CITY_TITLE') +'</span>';*/
        ...
        getZipLocationInput: function (node) {
            ...
            if (zipProperty) {
                ...
                label = BX.create('LABEL', {
                    ...
                    props: { className: 'bx-soa-custom-label hidden' },
                    html: labelTextHtml
                });
                input = BX.create('INPUT', {
                    props: {
                        ...
                        autocomplete: 'zip',
                        className: 'form-control bx-soa-customer-input bx-ios-fix hidden',
        ...
        editPaySystemInfo: function (paySystemNode) {
            ...
            if (this.result.PAY_FROM_ACCOUNT == 'Y' && currentPaySystem.CAN_PAY_BONUS == 'Y') {
            ...
            if (currentPaySystem) {
                ...
                if (currentPaySystem.DESCRIPTION.length > 0) {
                    title = BX.create('DIV', {
            ...
            if (this.result.PAY_FROM_ACCOUNT == 'Y') {
                $('.bx-soa-pp-inner-ps .bx-soa-pp-company-desc').after('<div class="bx-soa-pp-company-desc ">Вы можете оплатить до <strong>' + this.result.MAX_PERCENT_POINTS_PAY + '% (' + this.result.MAX_PAY_POINTS + ' руб.)</strong> суммы заказа бьюти-баллами.<br></div>');
            }
            if (this.result.PAY_FROM_ACCOUNT == 'Y' && this.result.PAY_CURRENT_ACCOUNT == 'Y') {
                $('.bx-soa-pp-inner-ps').append(`

					<div class="form-group bx-soa-customer-field pay-points-block">					
					
						<div class="soa-property-container">
							<input 
								type="text"							
								value="` + this.result.CURRENT_PAY_POINTS + `"
								name="PAY_FROM_ACCOUNT_POINTS" 
								id="PAY_FROM_ACCOUNT_POINTS"
								max="` + this.result.MAX_PAY_POINTS + `"
								min="0"
								step="1"
								class="form-control bx-soa-customer-input bx-ios-fix"
							>
							<a href="#" onclick="return false;" class="btn btn-default btn-md">Применить</a>
						</div>
					</div>
				`)
                BX.bind(document.getElementById('PAY_FROM_ACCOUNT_POINTS'), 'change', BX.proxy(this.selectPaySystem, this));
            }
        },
        ...
        getInnerPaySystem: function () {
            ...
            if (this.params.SHOW_PAY_SYSTEM_INFO_NAME == 'Y') {
                subTitle = BX.create('DIV', {
                    props: { className: 'bx-soa-pp-company-subTitle-block' },
                    children: [
                        ...
                        BX.create('label', {
                            ...
                            props: { className: 'bx-soa-pp-company-subTitle', },
                            // text: paySystem.NAME
                            text: 'Оплатить часть заказа бьюти-баллами',
                            children: [
                                BX.create('span', {
                                    text: 'Оплатить часть заказа бьюти-баллами',
                                }),
        ...
        editDeliveryInfo: function (deliveryNode) {
            ...
            if (currentDelivery.DESCRIPTION.length > 0) {
                title = BX.create('DIV', {
                    props: { className: 'bx-soa-pp-company-block' },
                    children: [
        ...
        editActivePropsBlock: function(activeNodeMode) {
            ...
            if (this.initialized.props) {
                ...
                // Создаем контент в контейнере внутри блока доставки
                var buyerContainer = BX('bx-soa-buyer-inside-delivery');
                if (buyerContainer) {
                    propsContent = buyerContainer.querySelector('.bx-soa-section-content');
                    if (!propsContent) {
                        propsContent = this.getNewContainer();
                        buyerContainer.appendChild(propsContent);
                    } else {
        ...
        editFadePropsBlock: function () {
            ...
            if (buyerContainer) {
                ...
                if (this.initialized.props) {
                    ...
                    BX.remove(BX.lastChild(buyerContainer));
                }
                newContent = this.getNewContainer();
                buyerContainer.appendChild(newContent);
                buyerContainer.style.display = 'block';
            } else {
                // Fallback к оригинальному поведению
                propsContent = this.propsBlockNode.querySelector('.bx-soa-section-content');
                if (this.initialized.props) {
                    this.propsHiddenBlockNode.appendChild(propsContent);
                } else {
                    this.editActivePropsBlock(false);
        ...
        getPropertyRowNode: function (property, propsItemsContainer, disabled) {
            ...
            if (disabled) {
                ...
                // Проверяем, нужно ли добавлять звездочку (исключаем ФИО и телефон)
                var settings = property.getSettings();
                var shouldShowStar = property.isRequired() && 
                                   settings.IS_PAYER !== 'Y' && 
                                   settings.IS_PHONE !== 'Y';
        ...
        editTotalBlock: function () {
            ...
            let bonusEarned = this.result.COUNT_BONUS_POINTS;
            this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_BONUS_EARNED_TEXT'), (bonusEarned ? bonusEarned : '0') + ' ' + BX.message('SOA_BONUS_EARNED_TEXT_MEASURE')));
            ...
            if (this.options.showPayedFromInnerBudget) {
                ...
                this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED + ' <span style="color: #999; text-decoration: line-through; font-size: 14px; margin-left: 10px;">' + (total.PRICE_WITHOUT_DISCOUNT || '2 500 ₽') + '</span>', { total: true }));
            }
            ...
            // Добавляем текст соглашения 
            this.totalInfoBlockNode.appendChild(
                BX.create('P', {
                    props: { className: 'bx-soa-legals-agreement' },
                    html: 'Оформляя заказ, вы соглашаетесь с <a href="/upload/oferta.pdf" target="_blank">Договором-офертой</a> и <a href="/include/licenses_detail.php" target="_blank">Политикой обработки персональных данных</a>.'
                })
            );
