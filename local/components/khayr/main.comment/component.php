...
foreach ($_POST as $key => $val) {
	$arResult["POST"][$key] = htmlspecialcharsbx(trim($val));
}

if (!$arResult["POST"]["EMAIL"] && isset($_COOKIE["KHAYR_COMMENT_EMAIL"]))
...
if (strlen($arResult["POST"]["ACTION"]) > 0) {
	...
	if (!$errors) {
		...
		$arResult["POST"]["MESSAGE"] = KhayRComment::emoji_to_unicode($arResult["POST"]["MESSAGE"]);
		$arResult["POST"]["DIGNITY"] = KhayRComment::emoji_to_unicode($arResult["POST"]["DIGNITY"]);
		$arResult["POST"]["FAULT"] = KhayRComment::emoji_to_unicode($arResult["POST"]["FAULT"]);
		switch ($arResult["POST"]["ACTION"]) {
			case "add":
				if (
					KhayRComment::Add(array(
