...
while ($arOrder = $dbOrder->Fetch()) {
	...
	foreach ($shipmentCollection as $shipment) {
		...
		foreach ($itemCollection as $shipmentItem) {
			...
			if ($isOrderCreatedByBuyer) {
				$totalSumByEshop += $basketItem->getQuantity() * $basketItem->getPrice();
			} else {
	...
	if ($isOrderCreatedByBuyer) {
		$arResult[$arOrder['RESPONSIBLE_ID']]['SUM_ESHOP'] += $totalSumByEshop;
	} else {
		$arResult[$arOrder['RESPONSIBLE_ID']]['SUM_RETAIL'] += $totalSumByRetail;
	}
...
foreach ($arResult as $key => $sum) {
	$arResult['ITEMS'][] = [
		"RESPONSIBLE_ID" => $key,
		"SUM_ESHOP" => $sum['SUM_ESHOP'],
		"SUM_RETAIL" => $sum['SUM_RETAIL'],
		"SUM_TOTAL" => $sum['SUM_ESHOP'] + $sum['SUM_RETAIL'],
		"ORDER_COUNT" => $sum['ORDERS_COUNT'],
		"MEDIAN_CHECK" => ($sum['SUM_ESHOP'] + $sum['SUM_RETAIL']) / $sum['ORDERS_COUNT'],
...
$arHeaders = array(
	...
	array("id" => "SUM_RETAIL", "content" => "Сумма продаж в розницу",  "sort" => "SUM_RETAIL", "default" => true, "align" => "left"),
	array("id" => "SUM_ESHOP", "content" => "Сумма продаж на сайте",  "sort" => "SUM_ESHOP", "default" => true, "align" => "left"),
...
while ($arResult = $dbResult->GetNext()) {
	...
	if (!empty($dbResult->arResult)) {
		...
		$row->AddViewField("SUM_ESHOP", CurrencyFormat($arResult['SUM_ESHOP'], "RUB"));
		$row->AddViewField("SUM_TOTAL", CurrencyFormat($arResult['SUM_TOTAL'], "RUB"));
		$row->AddViewField("MEDIAN_CHECK", CurrencyFormat(round($arResult['MEDIAN_CHECK'], 2), "RUB"));
