...
if(!class_exists("CNextCache")){
	class CNextCache {
		...
		public static function CIBlockElement_GetList($arOrder = array("SORT" => "ASC", "CACHE" => array("MULTI" => "Y", "GROUP" => array(), "RESULT" => array(), "TAG" => "", "PATH" => "", "TIME" => 36000000, "URL_TEMPLATE" => "")), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array()){
				...
				if($arGroupBy === array()){
					...
					if($bGetDetailPageUrlsArray){
						...
						foreach($arRes as $i => $arItem){
							if(is_array($arNewDetailPageUrls[$arItem["ID"]]) && count($arNewDetailPageUrls[$arItem["ID"]]) > 1){
		...
		public static function CIBlockSection_GetList($arOrder = array("SORT" => "ASC", "CACHE" => array("MULTI" => "Y", "GROUP" => array(), "RESULT" => array(), "TAG" => "", "PATH" => "", "TIME" => 36000000, "URL_TEMPLATE" => "")), $arFilter = array(), $bIncCnt = false, $arSelectFields = array(), $arNavStartParams = false){
		...
		public static function GroupArrayBy($arItems, $arParams){
			$arRes = array();
			$resultIDsCount = empty($arParams["RESULT"]) ? 0 : count($arParams["RESULT"]);
		...
		public static function GetIBlockCacheTag($IBLOCK_ID){
			if(!$IBLOCK_ID || is_array($IBLOCK_ID)){
				return false;
			}
			else{
