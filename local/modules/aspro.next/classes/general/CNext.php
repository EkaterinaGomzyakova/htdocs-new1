...
class CNext{
							...
							<p>
								...
								<?else:?>
									<?if($arResult['DISPLAY_PROPERTIES']['FORM_ORDER']['VALUE_XML_ID'] == 'YES'):?>
										<span>
											<span class="btn btn-default animate-load" data-event="jqm" data-param-form_id="<?=($arParams["FORM_ID_ORDER_SERVISE"] ? $arParams["FORM_ID_ORDER_SERVISE"] : 'SERVICES');?>" data-name="order_services" data-autoload-service="<?=$arResult['NAME']?>" data-autoload-project="<?=$arResult['NAME']?>" data-autoload-product="<?=$arResult['NAME']?>"><span><?=(strlen($arParams['S_ORDER_SERVISE']) ? $arParams['S_ORDER_SERVISE'] : Loc::getMessage('S_ORDER_SERVISE'))?></span></span>
										</span>
									<?endif;?>

									<?if($arResult['DISPLAY_PROPERTIES']['FORM_QUESTION']['VALUE_XML_ID'] == 'YES'):?>
										<span>
											<span class="btn btn-default white animate-load" data-event="jqm" data-param-form_id="ASK" data-autoload-need_product="<?=$arResult['NAME']?>" data-name="question"><span><?=(strlen($arParams['S_ASK_QUESTION']) ? $arParams['S_ASK_QUESTION'] : Loc::getMessage('S_ASK_QUESTION'))?></span></span>
										</span>
									<?endif;?>
								<?endif;?>
							</p>
	...
	public static function ShowMobileMenuContacts(){
		...
		<?if($iCountPhones): // count of phones?>
			...
			<?if($arRegion):?>
			<?$frame = new \Bitrix\Main\Page\FrameHelper('mobile-phone-block'.$iCalledID.$regionID);?>
			<?$frame->begin();?>
			<?endif;?>
		...
		<div class="contacts">
			...
			<?if($arRegion):?>
			<?$frame = new \Bitrix\Main\Page\FrameHelper('mobile-contact-block'.$regionID);?>
			<?$frame->begin();?>
			<?endif;?>
	...
	public static function ShowMobileRegions(){
		...
		if($arRegion):
			...
			<?Bitrix\Main\Page\Frame::getInstance()->startDynamicWithID('mobile-region-block'.$iCalledID.$regionID);?>
			<!-- noindex -->
			...
			<!-- /noindex -->
			<?Bitrix\Main\Page\Frame::getInstance()->finishDynamicWithID('mobile-region-block'.$iCalledID.$regionID);?>
		<?endif;
	}
		...
		<?elseif($optionType == "note"):?>
			<?if($optionCode == 'GOALS_NOTE'){
				...
				if(\Bitrix\Main\Loader::includeModule('form'))
				{
					if($optionsSiteID)
					{
						if($arForms = CNextCache::CForm_GetList(array('by' => 's_id', 'CACHE' => array('TAG' => 'forms')), 'asc', array('SITE' => $optionsSiteID, 'SITE_EXACT_MATCH' => 'Y'), $is_filtered))
						{
				...
				<?elseif($optionType === 'includefile'):?>
					...
					foreach($arOption['INCLUDEFILE'] as $includefile){
						...
						if(strpos($includefile, '#') === false){
							...
							$href = (!strlen($includefile) ? "javascript:;" : "javascript: new BX.CAdminDialog({'content_url':'/bitrix/admin/public_file_edit.php?site=".$arTab['SITE_ID']."&bxpublic=Y&from=includefile&path=".$includefile."&lang=".LANGUAGE_ID."&template=".$template."&subdialog=Y','width':'1009','height':'503'}).Show();");
	...
	public static function Start($siteID){
		...
		if(CModule::IncludeModuleEx(self::moduleID) == 1)
		{
			...
			$APPLICATION->SetAdditionalCSS((CMain::IsHTTPS() ? 'https' : 'http').'://fonts.googleapis.com/css?family='.$font_family.'&display=swap');*/
			...
			//custom
			$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/cnext.bundle.js');
	...
	public static function checkBgImage($siteID){
		...
		if($arBanner === NULL)
		{
			...
			if($arRegion && isset($arTheme['REGIONALITY_FILTER_ITEM']) && $arTheme['REGIONALITY_FILTER_ITEM'] == 'Y')
	...
	public static function IsPersonalPage($page = ''){
		...
		if(!isset($result))
		{
			if(!$page)
			{
				...
				$url_link = $arOptions['PERSONAL_PAGE_URL'];
			}
			$result = CSite::InDir($url_link);
		}
					...
					<?foreach($arItem['PRICE_MATRIX']['COLS'] as $arPriceGroup):?>
						...
						<?foreach($arItem['PRICE_MATRIX']['MATRIX'][$arPriceGroup['ID']] as $key => $arPrice):?>
							<?if($iCountPriceInterval > 1):?>
								...
								<?if($arPrice["PRICE"] > $arPrice["DISCOUNT_PRICE"]){?>
									<div class="price" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["DISCOUNT_PRICE"];?>">
										<?if(strlen($arPrice["DISCOUNT_PRICE"])):?>
											<span class="values_wrapper"><?=\Aspro\Functions\CAsproItem::getCurrentPrice("DISCOUNT_PRICE", $arPrice);?></span><?if(($arParams["SHOW_MEASURE"]=="Y") && $strMeasure):?><span class="price_measure">/<?=$strMeasure?></span><?endif;?>
										<?endif;?>
									</div>
								...
								<?}else{?>
									<div class="price" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["DISCOUNT_PRICE"];?>">
										<span><span class="values_wrapper"><?=\Aspro\Functions\CAsproItem::getCurrentPrice("PRICE", $arPrice);?></span><?if(($arParams["SHOW_MEASURE"]=="Y") && $strMeasure):?><span class="price_measure">/<?=$strMeasure?></span><?endif;?></span>
									</div>
								<?}?>
							<?if($iCountPriceInterval > 1):?>
								</div>
							<?else:
								if($arParams['SHOW_DISCOUNT_PERCENT'] == 'Y' && $arPrice["PRICE"] > $arPrice["DISCOUNT_PRICE"]):?>
									...
									<div class="sale_block">
										<span class="title"><?=GetMessage("CATALOG_ECONOMY");?></span> <div class="text"><span class="values_wrapper" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=(($arPrice["PRICE"] - $arPrice["DISCOUNT_PRICE"])*$ratio);?>"><?= CurrencyFormat(($arPrice["PRICE"] - $arPrice["DISCOUNT_PRICE"]), $arPrice['CURRENCY'])?></span></div>
									</div>
								<?endif;?>
							<?endif;?>
						<?endforeach;?>
			...
			if(isset($arItem['PRICE_MATRIX']) && $arItem['PRICE_MATRIX'])
			{
				ob_start();
					...
					<div class="with_matrix">
						<div class="price">
							<div class="price_value_block"><?=\Aspro\Functions\CAsproItem::getCurrentPrice("PRICE", $arItem['ITEM_PRICES'][$priceSelected]);?></div><?if (($arParams['SHOW_MEASURE'] == 'Y') && $arItem['CATALOG_MEASURE_NAME']):?><span class="price_measure">/<?=$arItem['CATALOG_MEASURE_NAME']?></span><?endif;?>
						</div>
		...
		if($arParams['STORES'])
		{
			...
			if($arItem['OFFERS'])
			{
				foreach($arItem['OFFERS'] as $arOffer)
				{
					...
					$totalCount += $quantity - $arProduct['QUANTITY_RESERVED'];
				}
			}
			else
			{
    ...
    /**
     * Проверяет товары, которых нет на складе, но по которым есть непроведенные документы прихода
     *
     * @param ?int $productId - ID элемента каталога
     * @return array
     */
    private static function checkCommigSoonArrivals(?int $productId)
    {
        $arElements = [];
        $res = CCatalogStoreDocsElement::getList(
            ...
            false,
            false,
            ['ID', 'DOC_ID', 'ELEMENT_ID']
        );
        while ($arFields = $res->Fetch()) {
            $arElements[$arFields['DOC_ID']][$arFields['ID']] = $arFields;
        }
        ...
        while ($arFields = $res->Fetch()) {
            foreach ($arElements[$arFields['ID']] as $arItem) {
                if ($arItem['DOC_ID'] == $arFields['ID']) {
                    $arResult[$arItem['ELEMENT_ID']] = [
                        'MESSAGE' => 'Скоро в наличии'
                    ];
                }
            }
        }
	...
	public static function GetQuantityArray($totalCount, $arItemIDs = array(), $useStoreClick="N"){
		...
		$arQuantityOptions["USE_WORD_EXPRESSION"]  = WL\SnailShop::UserIsStaff() ? "N" : Option::get(self::moduleID, "USE_WORD_EXPRESSION", "Y", SITE_ID);
		...
		if($arQuantityRights["SHOW_QUANTITY"]){
            ...
            $justExists = false;
			$isAvailableOnNotDefaultStore = false;
			if($totalCount > 0){
				if($totalCount === 1) {
					$totalText = GetMessage("CUSTOM_EXPRESSION_FOR_LAST_ONE");
				} else {
				...
				if($useStoreClick=="Y"){
					...
					$arStoresId = [];
					$dbStores = CCatalogStore::GetList([], ['IS_DEFAULT' => 'N', 'ACTIVE' => 'Y'], false, false, ['ID']);
					while($arStore = $dbStores->Fetch()) {
						$arStoresId[] = $arStore['ID'];
					}
					if(!empty($arStoresId)) {
						$arStoreProduct = CCatalogStoreProduct::GetList([], ['STORE_ID' => $arStoresId, '>AMOUNT' => 0, 'PRODUCT_ID' => $arItemIDs['CATALOG_ITEM_ID']], false, false, ['ID', 'AMOUNT'])->Fetch();
						if($arStoreProduct) {
							$isAvailableOnNotDefaultStore = true;
							$totalText = GetMessage("CUSTOM_EXPRESSION_AVAILABLE_ON_NOT_DEFAULT_STORE");
						} else {
            ...
            if (($totalCount == 0) && !empty($arItemIDs['CATALOG_ITEM_ID']) && !$isAvailableOnNotDefaultStore) {
                $arArrival = self::checkCommigSoonArrivals($arItemIDs['CATALOG_ITEM_ID']);
                if (!empty($arArrival[$arItemIDs['CATALOG_ITEM_ID']])) {
                    $totalText = $arArrival[$arItemIDs['CATALOG_ITEM_ID']]['MESSAGE'];
                }
            }
			...
			if($totalCount > 0) {
				...
				if($isAvailableOnNotDefaultStore) {
					$iconClass .= 'last';
				} else {
					$iconClass .= 'order';
				}
            ...
            $totalHTMLs .= '<span class="icon '. $iconClass .'"></span><span class="value">'. $totalText .'</span>';
			$totalHTMLs .='</div>';
            if ($justExists) {
                $totalHTMLs = '';
            }
	...
	public static function GetAvailableStore($totalCount = 0, $arItemIDs=array(), $detail=false){
	...
	public static function GetSKUPropsArray(&$arSkuProps, $iblock_id=0, $type_view="list", $hide_title_props="N", $group_iblock_id="N"){
		...
		'<span class="'.$class_title.'">'.$hint_block.'<span>'.htmlspecialcharsex($arProp['NAME']).'</span></span>'.
		'<div class="bx_scu_scroller_container"><div class="bx_scu"><ul id="#ITEM#_prop_'.$arProp['ID'].'_list" class="owl-carousel">';
						}
						else
						{
							$templateRow .= '<div class="bx_item_detail_size" id="#ITEM#_prop_'.$arProp['ID'].'_cont">'.
		...
		'<div class="bx_size_scroller_container"><div class="bx_size"><ul id="#ITEM#_prop_'.$arProp['ID'].'_list">';
						...
						foreach ($arProp['VALUES'] as $arOneValue){
							if($arOneValue['ID']>0){
								...
								{
									$templateRow .= '<li data-treevalue="'.$arProp['ID'].'_'.$arOneValue['ID'].'" data-showtype="li" data-image="'.$arOneValue['PICT']['SRC'].'" data-onevalue="'.$arOneValue['ID'].'"><i title="'.$arProp['NAME'].': '.$arOneValue['NAME'].'"></i>'.
	...
	'</div></div>';
                    ...
                    $classHide = '';
                    if ($arProp['CODE'] == 'NOMINAL') {
                        $request = Context::getCurrent()->getRequest();
                        $templateRow .= '<select class="select-list-element">';
                        foreach ($arProp['VALUES'] as $arOneValue){
                            if($arOneValue['ID']>0) {
                                $name = htmlspecialcharsbx($arOneValue['NAME']);

                                $selected = '';
                                if ($request->get('variant') == $name) {
                                    $selected = 'selected';
                                }

                                $templateRow .= '<option value="' . $arOneValue['ID'] . '"' . $selected . '>';
                                $templateRow .= htmlspecialcharsbx($arOneValue['NAME']);
                                $templateRow .= '</option>';
                            }
                        }
                        $templateRow .= '</select>';
                    ...
                    $templateRow .= '<ul ' . $classHide . ' id="#ITEM#_prop_'.$arProp['ID'].'_list">';
                    foreach ($arProp['VALUES'] as $arOneValue){
                        if($arOneValue['ID']>0){
                            $arOneValue['NAME'] = htmlspecialcharsbx($arOneValue['NAME']);
                            $templateRow .= '<li data-treevalue="'.$arProp['ID'].'_'.$arOneValue['ID'].'" data-showtype="li" data-onevalue="'.$arOneValue['ID'].'" title="'.$arProp['NAME'].': '.$arOneValue['NAME'].'"><i></i><span class="cnt">'.$arOneValue['NAME'].'</span></li>';
	...
	'<div class="bx_size_scroller_container"><div class="bx_size"><ul id="#ITEM#_prop_'.$arProp['ID'].'_list">';
					...
					foreach ($arProp['VALUES'] as $arOneValue){
						if($arOneValue['ID']>0){
							...
							if(isset($arOneValue['PICT']['SRC']) && $arOneValue['PICT']['SRC'] && $isHasPicture)
							{
								$templateRow .= '<li data-treevalue="'.$arProp['ID'].'_'.$arOneValue['ID'].'" data-image="'.$arOneValue['PICT']['SRC'].'" data-showtype="li" data-onevalue="'.$arOneValue['ID'].'"><i title="'.$arProp['NAME'].': '.$arOneValue['NAME'].'"></i>'.
		'<span class="cnt1"><span class="cnt_item" style="background-image:url(\''.$arOneValue['PICT']['SRC'].'\');" title="'.$arProp['NAME'].': '.$arOneValue['NAME'].'"></span></span></li>';
							}
							else
							{
			...
			if($bPriceExists = isset($arItem["MIN_PRICE"]) && $arItem["MIN_PRICE"]["VALUE"] > 0){
						...
						if($arAddToBasketOptions["BUYMISSINGGOODS"] == "ADD" /*|| $arItem["CAN_BUY"]*/){
							if($arItem["CAN_BUY"]){
								...
								if($arAddToBasketOptions["BUYMISSINGGOODS"] == "SUBSCRIBE" && $arItem["CATALOG_SUBSCRIBE"] == "Y"){
									...
									$buttonHTML = '<span class="'.$class_btn.' to-order btn btn-default white grey transition_bg transparent animate-load" data-event="jqm" data-param-form_id="TOORDER" data-name="toorder" data-autoload-product_name="'.str_replace('&quot;', '&#039;', $arItem["NAME"]).'" data-autoload-product_id="'.$arItem["ID"].'"><i></i><span>'.$buttonText[0].'</span></span>';
				...
				if($bDetail && $arItem["FRONT_CATALOG"] == "Y"){
					...
					if($arAddToBasketOptions["BUYNOPRICEGGOODS"] == "ORDER"){
						$buttonText = array($arAddToBasketOptions['EXPRESSION_ORDER_BUTTON']);
						$buttonHTML = '<span class="'.$class_btn.' to-order btn btn-default white grey transition_bg transparent animate-load" data-event="jqm" data-param-form_id="TOORDER" data-name="toorder" data-autoload-product_name="'.$arItem["NAME"].'" data-autoload-product_id="'.$arItem["ID"].'"><i></i><span>'.$buttonText[0].'</span></span>';
						if($arAddToBasketOptions['EXPRESSION_ORDER_TEXT']){
							$buttonHTML .='<div class="more_text">'.$arAddToBasketOptions['EXPRESSION_ORDER_TEXT'].'</div>';
						}
					}
	...
	public static function SetJSOptions(){
		global $APPLICATION, $STARTTIME, $arSite;
	...
	public static function showAddress($class = ''){
		...
		<?if($arRegion):?>
		<?$frame = new \Bitrix\Main\Page\FrameHelper('address-block'.$iCalledID.$regionID);?>
		<?$frame->begin();?>
		<?endif;?>
	...
	public static function showEmail($class = ''){
		...
		<?if($arRegion):?>
		<?$frame = new \Bitrix\Main\Page\FrameHelper('email-block'.$iCalledID.$regionID);?>
		<?$frame->begin();?>
		<?endif;?>
	...
	public static function ShowHeaderPhones($class = '', $bFooter = false){
		...
		<?if($arRegion):?>
		<?$frame = new \Bitrix\Main\Page\FrameHelper('header-allphones-block'.$iCalledID.$regionID);?>
		<?$frame->begin();?>
		<?endif;?>
	...
	public static function showFooterPhone(){
		...
		<?if($arRegion):?>
		<?$frame = new \Bitrix\Main\Page\FrameHelper('footer-allphones-block'.$iCalledID.$regionID);?>
		<?$frame->begin();?>
		<?endif;?>
	...
	public static function newAction($action = "unknown"){
		// $socket = fsockopen('localhost', 80, $errno, $errstr, 10);
		// if($socket)
		// {
	...
	public static function setFooterTitle(){
		...
		if($arRegion)
		{
			CNextRegionality::addSeoMarks(array('#ID#' => 'ID'));
	...
	public static function showFooterBasket(){
		...
		<?if($arTheme['ORDER_BASKET_VIEW']['VALUE'] == 'BOTTOM'):?>
		...
		<?elseif($arTheme['ORDER_BASKET_VIEW']['VALUE'] != 'NORMAL'):?>
			<div class="basket_wrapp <?=strtolower($arTheme['ORDER_BASKET_VIEW']['VALUE']);?> basket_fill_<?=$arTheme['ORDER_BASKET_COLOR']['VALUE'];?>">
	...
	public static function drawShopDetail($arShop, $arParams, $showMap="Y"){
		...
		if(is_array($arShop)){
			...
			<div class="wrapper_inner shop detail">
				...
				<div class="clearboth"></div>
				<!-- noindex--><a rel="nofollow" href="<?=$arShop['LIST_URL']?>" class="back-url url-block"><i class="fa fa-angle-left"></i><span><?=GetMessage('BACK_STORE_LIST')?></span></a><!-- /noindex-->
			</div>
			<div class="clearboth"></div>
			<?
		}
