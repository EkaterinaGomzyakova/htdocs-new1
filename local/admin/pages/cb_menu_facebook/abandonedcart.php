...
$APPLICATION->SetTitle("Пользователи с брошенными корзинами Facebook");
...
$arMergedBasketUsers = [];
$dbResultList = CSaleBasket::GetLeave(array($by => $order), ['!USER_ID' => false]);
while($arBasket = $dbResultList->Fetch()) {
    $arMergedBasketUsers[$arBasket['FUSER_ID']] = $arBasket;
}
$nav->setRecordCount($dbResultList->SelectedRowsCount());

$usersID = array_column($arMergedBasketUsers, 'USER_ID');
...
foreach ($arMergedBasketUsers as $key => $buyerStat) {
    if(empty($arUserProps[$buyerStat['USER_ID']])){
        ...
        $arMergedBasketUsers[$key]['USER_PROPS']['LOCATION'] = $location;
    }
}
...
$arResult['ITEMS'] = [];
foreach ($arMergedBasketUsers as $buyerStat) {
    $item = $buyerStat;
